...
def load_semantic_node_specs() -> dict[str, type]:
    ...
    for path in sorted(specs_dir.glob("*.yaml")):
        ...
        specs = payload.get("modules", [payload])
        for raw in specs:
            plugin = raw["plugin"]
            model = raw["model"]
            kind = raw["kind"]
            ...
            class_name = raw.get("class_name") or f"{_sanitize_name(plugin)}{_sanitize_name(model)}Node"
            key = f"{plugin}/{model}"
            if key in registry:
                raise ValueError(f"Duplicate semantic node spec for {key} in {path}")
            registry[key] = type(class_name, (base,), attrs)
    return registry
